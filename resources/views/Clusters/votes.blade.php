...
@section('title', __("site.Clusters Votes Count"))
@section('id', 'Votes-percentages')

@push('style')
    <link rel="stylesheet" href="{{asset('/css/Votes-percentages.css')}}">
@endpush
@section('content')
    <x-layout.header-image title="{{__('site.Clusters Votes Count')}}"></x-layout.header-image>
    ...
    <section>
        <div class="container filter-container">
            <form action="">
                ...
                <input type="text" name="search" value="{{$form_data['search']}}" />
    ...
    <section>
        <div class="container Votes-percentages-container">
            @forelse($clusters as $cluster)
                <div class="Votes-percentages-card">
                    <div>
                        <picture>
                            <x-curator-glider
                                :media="$cluster->thumbnail_image_id"
                            />
                        </picture>
                        <h2>{{$cluster->title}}</h2>
                    </div>
                    <a href="{{route('clusters.votes.show', ['cluster' => $cluster])}}" class="secondary-btn d-flex align-items-center gap-3">
                        @lang('site.View Percentages')
                        <i class="fa-solid fa-arrow-left-long pt-1"></i>
                    </a>
                </div>
            @empty
                <div class="d-flex w-100 justify-content-center mt-3">
                    <h3>
                        @lang('site.No Candidates With This Filters')
                    </h3>
                </div>
            @endforelse
        </div>
