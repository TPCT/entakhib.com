...
@section('title', __("site.Parties"))
@section('id', 'GeneralElectoralLists')

@push('style')
    <link rel="stylesheet" href="{{asset('/css/GeneralElectoralLists.css')}}">
@endpush
@section('content')
    <x-layout.header-image title="{{__('site.Parties')}}"></x-layout.header-image>
    <x-layout.share></x-layout.share>

    <section class="mb-3 container">
        <div class="container GeneralElectoralLists-container">
            @forelse($parties as $party)
                <form method="get" class="GeneralElectoralLists-card" id="parties-election-form" action="{{route('profile.vote', ['type' => 'party'])}}">
                    @csrf
                    <picture>
                        <x-curator-glider
                    ...
                    <div class="GeneralElectoralLists-card-content">
                        <div>
                            <h2>{{$party->title}}</h2>
                            {!! $party->second_title !!}
                        </div>
                        <div class="mb-2 GeneralElectoralLists-btn-group ">
                            <a href="{{route('parties.show', ['party' => $party])}}" class="secondary-btn show-ones-btn"> @lang('site.Show Candidates')
                            ...
                            <input type="hidden" value="{{$party->id}}" name="party_id" class="">
                            @if ($party->candidates->count() && $can_vote)
                                <button type="submit" class="main-btn position-relative eligate-btn election-button">
                                    @lang('site.Elect')
                                </button>
                            @endif
                        </div>
